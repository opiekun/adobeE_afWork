...
class GetCustomers extends AbstractSync
{
    ...
    public function execute()
    {
        try {
            ...
            // Exclude customer with group ids
            $custGroupIdsIgnore = $this->helper->getCustomerGroupIdsIgnore();
            $custGroupIds = [];
            if (isset($custGroupIdsIgnore)) {
                $custGroupIds = explode(",", $custGroupIdsIgnore);
            }
            ...
            if ($custGroupIds) {
                $collection->addFieldToFilter('group_id', array('nin' => $custGroupIds));
            }
        ...
        if ($custGroupIds) {
            foreach ($custGroupIds as $customerGroupId) {
                if (array_key_exists($customerGroupId, $custGroups)) {
                    unset($custGroups[$customerGroupId]);
                }
            }
        }
