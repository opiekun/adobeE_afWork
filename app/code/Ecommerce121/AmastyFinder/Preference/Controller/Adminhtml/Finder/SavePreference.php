...
namespace Ecommerce121\AmastyFinder\Preference\Controller\Adminhtml\Finder;

use Amasty\Finder\Api\Data\DropdownInterface;
use Amasty\Finder\Controller\Adminhtml\Finder\Save;

class SavePreference extends Save
{
    /**
     * @inheirtDoc
     */
                ...
                if ($finderId) {
                    foreach ($model->getDropdowns() as $dropdown) {
                        ...
                        $dropdown->addData([
                            ...
                            'hide_in_form' => $model->getData($prefix . '_hide_in_form'),
                            'required' => $model->getData($prefix . '_required'),
                        ]);
                        $dropdown->save();
                    }
                } else {
